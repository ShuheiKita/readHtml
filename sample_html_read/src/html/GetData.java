...
public class GetData {
	...
	public static void readWorks(Calendar cal) throws Exception{
		...
		try {
		    ...
		    for(Element work : works){
		    	...
		    	//サークルDLSiteID
		    	obj.getCircle().setDLSiteId(getCircleDLSiteId(work));
		    	 ...
		    	 */
		    	if(obj.isPerfect() && ConnectorDB.isDlTable(obj)){
	...
	/**
	 * サークルDLSiteID取得　＠一覧ページ.
	 * @param body
	 * @return
	 */
	private static String getCircleDLSiteId(Element body) throws Exception{
		try{
			List<Element> tmp = body.getElementsByClass("maker_name");
